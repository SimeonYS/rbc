...
class RbvPipeline:
    ...
    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `rbc`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM rbc WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO rbc (date, title, link, content)"
